...
We find that the model does maintain a high precision as the recall value increases. However, it was not able to detect a lot of interlinear annotations. We believe that this is because most of our data consists of marginalia, so there is a bias towards detecting marginalia than interlinear data. We believe that if interlinear data was more equally represented with marginalia, then average precision would improve drastically. Overall, we believe that most scholars would be more interested in marginalia as it contains more content than interlinear annotations, and we expect our users to be happy with using this model to speed up the process of finding all annotations in IIIF manifests.
